...
import org.ventura.cpe.utils.Utils;
...
import static org.ventura.soluciones.commons.config.IUBLConfig.*;
...
public abstract class UBLBasicHandler {
    ...
    protected List<NoteType> getNotesWithIfSentence(List<TransaccionPropiedades> transaccionPropiedadesList) {
        ...
        for (TransaccionPropiedades transaccionPropiedad : transaccionPropiedadesList) {
            if (transaccionPropiedad.getTransaccionPropiedadesPK().getId().substring(0, 2).equals("10") ||
                    transaccionPropiedad.getTransaccionPropiedadesPK().getId().substring(0, 2).equals("20")) {
                if (logger.isDebugEnabled()) {
                    logger.debug("getNotes() [" + this.identifier + "] xxxxxxxxxxxxxxx Agregando PROPIEDAD(" + transaccionPropiedad.getTransaccionPropiedadesPK().getId() + ", " + transaccionPropiedad.getValor() + ")");
                }
                NoteType note = new NoteType();
    ...
    protected List<PaymentTermsType> getPaymentTermsInvoice(boolean contado, Transaccion transaccion) {
        ...
        try {
            ...
            if (contado) {
                ...
                amountType.setCurrencyID(transaccion.getDOCMONCodigo());

                /**MONTO PAGO*/
                BigDecimal sumaTotalCuotas = BigDecimal.ZERO;
                for (TransaccionCuotas lineaCuota : transaccion.getTransaccionCuotas()) {
                    BigDecimal pagoCuota = Optional.<BigDecimal>ofNullable(lineaCuota.getMontoCuota().setScale(2, 4)).orElse(BigDecimal.ZERO);
                    sumaTotalCuotas = sumaTotalCuotas.add(pagoCuota);
                }
                amountType.setValue(sumaTotalCuotas.setScale(2, 4));
                ...
                for (TransaccionCuotas lineaCuota : transaccion.getTransaccionCuotas()) {
                    ...
                    amountType2.setCurrencyID(transaccion.getDOCMONCodigo());

                    /**MONTO PAGAR POR CUOTA**/
                    BigDecimal pagoCuota = Optional.<BigDecimal>ofNullable(lineaCuota.getMontoCuota().setScale(2, 4)).orElse(BigDecimal.ZERO);
                    amountType2.setValue(pagoCuota.setScale(2, 4));
    ...
    protected List<PaymentTermsType> getPaymentTermsNoteCredit(Transaccion transaccion) {
        ...
        try {
            ...
            for (TransaccionCuotas lineaCuota : transaccion.getTransaccionCuotas()) {
                ...
                amountType.setValue(sumTotalCuotas.setScale(2, 4));
    ...
    protected List<PaymentTermsType> getPaymentTermsNoteCreditLast(boolean contado, Transaccion transaccion) {
        ...
        try {
            ...
            if (contado) {
                ...
                AmountType amountType = new AmountType();
                amountType.setCurrencyID(transaccion.getDOCMONCodigo());
                if (transaccion.getDOCCodigo().equals("07")) {
                    BigDecimal pago = Optional.<BigDecimal>ofNullable(transaccion.getImporteDOCRef()).orElse(BigDecimal.ZERO);
                    amountType.setValue(pago.setScale(2, 4));
                }
                /*} else {
                ...
                if (transaccion.getDOCCodigo().equals("07")) {
                    if (transaccion.getREFDOCMotivCode().equals("13")) {
...
//            party.getPartyTaxScheme().add(partyTaxScheme);
    ...
    protected ShipmentType getShipment(TransaccionGuiaRemision transaccionGuiaRemision, Transaccion transaccion) throws UBLDocumentException {
        ...
        try {
            ...
            if (transaccion.getTransaccionGuiaRemision().getCodigoMotivo().equals("02") || transaccion.getTransaccionGuiaRemision().getCodigoMotivo().equals("08")) {
                ...
                AddressTypeCodeType addressTypeCode = new AddressTypeCodeType();
                addressTypeCode.setListID(transaccion.getDocIdentidadNro());
                ...
                if (transaccion.getSNDocIdentidadTipo().equals("6")) {
                    AddressTypeCodeType addressTypeCode = new AddressTypeCodeType();
                    addressTypeCode.setListID(transaccion.getSNDocIdentidadNro());
                    addressTypeCode.setListAgencyName(IUBLConfig.SCHEME_AGENCY_NAME_PE_SUNAT);
                    addressTypeCode.setListName(IUBLConfig.ESTABLECIMIENTOS_GUIAS);
                    addressTypeCode.setValue("0");
                    deliveryAddress.setAddressTypeCode(addressTypeCode);
            ...
            if (transaccion.getTransaccionGuiaRemision().getCodigoMotivo().equals("02") || transaccion.getTransaccionGuiaRemision().getCodigoMotivo().equals("08")) {
                ...
                addressTypeCodeDespatche.setListID(transaccion.getSNDocIdentidadNro());
                ...
                /* <cac:Shipment><cac:Delivery><cac:Despatch><cac:DespatchAddress><cbc:AddressTypeCode>*/
                AddressTypeCodeType addressTypeCodeDespatche = new AddressTypeCodeType();
                addressTypeCodeDespatche.setListID(transaccion.getDocIdentidadNro());
                addressTypeCodeDespatche.setListAgencyName(IUBLConfig.SCHEME_AGENCY_NAME_PE_SUNAT);
                addressTypeCodeDespatche.setListName(IUBLConfig.ESTABLECIMIENTOS_GUIAS);
                addressTypeCodeDespatche.setValue("0");
    ...
    protected ShipmentType getShipmentCarrier(TransaccionGuiaRemision transaccionGuiaRemision, Transaccion transaccion) throws UBLDocumentException {
        if (logger.isDebugEnabled()) {
    ...
    protected PartyType getCarrierParty(TransaccionGuiaRemision transaccionGuiaRemision) throws UBLDocumentException {
        PartyType carrierParty = new PartyType();
    ...
    protected TaxTotalType getTaxTotalV21(Transaccion transaccion, List<TransaccionImpuestos> transaccionImpuestos, BigDecimal impuestoTotal, String currencyCode) throws UBLDocumentException {
        ...
        try {
            ...
            for (TransaccionImpuestos transaccionImpuesto : transaccionImpuestos) {
                ...
                if (isImpuestoBolsa) {
                    taxAmount.setValue(transaccionImpuesto.getValorVenta().setScale(2, RoundingMode.HALF_UP));
                } else {
    ...
    protected MonetaryTotalType getMonetaryTotal(final Transaccion transaccion, final BigDecimal lineExtensionAmountValue, final BigDecimal taxInclusiveAmountValue, final boolean noContainsFreeItem, final BigDecimal chargeTotalAmountValue, final BigDecimal prepaidAmountValue, final BigDecimal payableAmountValue, final BigDecimal descuento, final String currencyCode, final boolean isInvoiceOrBoleta) throws UBLDocumentException {
        ...
        if (this.logger.isDebugEnabled()) {
            this.logger.debug((Object)("+getMonetaryTotal() [" + this.identifier + "]"));
        }
        ...
        if (isInvoiceOrBoleta && noContainsFreeItem) {
            ...
            if (transaccion.getANTICIPOMonto().intValue() > 0 && !transaccion.getTipoOperacionSunat().startsWith("02")) {
            ...
            /** */
                ...
                for (int i = 0; i < transaccion.getTransaccionImpuestosList().size(); ++i) {
                    if (transaccion.getTransaccionImpuestosList().get(i).getNombre().equals("IGV") ||
                            transaccion.getTransaccionImpuestosList().get(i).getNombre().equals("GRA")) {
                        existIGV = true;
                        ...
                        suma = suma.add(transaccion.getTransaccionImpuestosList().get(i).getMonto()).setScale(2, RoundingMode.HALF_UP);
            ...
            if (transaccion.getTransaccionImpuestosList() != null) {
                for (int j = 0; j < transaccion.getTransaccionImpuestosList().size(); ++j) {
                    ...
                    if(transaccion.getTransaccionImpuestosList().get(j).getNombre().equals("EXP")){
                        existEXP = true;
                        taxInclusiveAmount.setValue(lineExtensionAmountValue.add(transaccion.getTransaccionImpuestosList().get(j).getMonto()).setScale(2, RoundingMode.HALF_UP));
                        taxInclusiveAmount.setCurrencyID(CurrencyCodeContentType.valueOf(currencyCode).value());
                    }
                }
            }
            ...
            if(validarResta(transaccion)){
                BigDecimal taxInclusiveValue = taxInclusiveAmount.getValue().subtract(transaccion.getANTICIPOMonto());
                String taxInclusiveValueString = taxInclusiveValue.toString();
                if (this.logger.isDebugEnabled()) {
                    this.logger.debug((Object)("+getMonetaryTotal() [" + this.identifier + "] <cbc:TaxInclusiveAmount>"+taxInclusiveValueString+""));
                }
        ...
        if (null != chargeTotalAmountValue && chargeTotalAmountValue.compareTo(BigDecimal.ZERO) > 0) {
            if (this.logger.isInfoEnabled()) {
                this.logger.info((Object)("getMonetaryTotal() [" + this.identifier + "] La transaccion tiene un TOTAL DE OTROS CARGOS."));
            }
        ...
        if (null != prepaidAmountValue && prepaidAmountValue.compareTo(BigDecimal.ZERO) > 0) {
            if (this.logger.isInfoEnabled()) {
                this.logger.info((Object)("getMonetaryTotal() [" + this.identifier + "] La transaccion tiene un TOTAL DE ANTICIPOS."));
            }
        ...
        if (transaccion.getTransaccionPropiedadesList().size() == 2 && transaccion.getTransaccionPropiedadesList().get(1).getDescription().contains("GRATUITA")) {
            final LineExtensionAmountType lineExtensionAmount = new LineExtensionAmountType();
            lineExtensionAmount.setValue(lineExtensionAmountValue.setScale(2, RoundingMode.HALF_UP));
            lineExtensionAmount.setCurrencyID(CurrencyCodeContentType.valueOf(currencyCode).value());
            monetaryTotal.setLineExtensionAmount(lineExtensionAmount);
        ...
        if(transaccion.getFEFormSAP().contains("exportacion") || transaccion.getTipoOperacionSunat().startsWith("02")){
            ...
            allowanceTotalAmountType.setValue(transaccion.getDOCDescuento().setScale(2, RoundingMode.HALF_UP));
            monetaryTotal.setAllowanceTotalAmount(allowanceTotalAmountType);
        }
        return monetaryTotal;
    }

    protected Boolean validarResta(final Transaccion transaccion){
        if (transaccion.getMontoRetencion() != null && transaccion.getMontoRetencion().compareTo(BigDecimal.ZERO) > 0 && transaccion.getANTICIPOMonto().compareTo(BigDecimal.ZERO) > 0) {
            if(transaccion.getFEFormSAP().contains("exportacion") || transaccion.getTipoOperacionSunat().startsWith("02")){
                return true;
            }
        }
    ...
    protected DeliveredQuantityType getDeliveredQuantity(BigDecimal value, String unitCodeValue) throws Exception {
        ...
        /** Harol 02-11-2023 el valor se redondeaba a entero*/
    ...
    protected PricingReferenceType getPricingReference(String priceTypeCodeValue, BigDecimal priceAmountValue, String currencyCode, TransaccionLineas linea) {
        ...
        priceAmount.setValue(priceAmountValue.setScale(6, BigDecimal.ROUND_HALF_UP));
...
//            }
            ...
            for (TransaccionLineaImpuestos lineaImpuesto : transaccionLineaImpuestos) {
                ...
                if (tributo.equalsIgnoreCase(TAX_TOTAL_IGV_ID) || tributo.equalsIgnoreCase(TAX_TOTAL_ISC_ID) || tributo.equalsIgnoreCase(TAX_TOTAL_EXP_ID) || tributo.equalsIgnoreCase(IUBLConfig.TAX_TOTAL_GRT_ID)
                        || tributo.equalsIgnoreCase(TAX_TOTAL_EXO_ID) || tributo.equalsIgnoreCase(TAX_TOTAL_INA_ID) || tributo.equalsIgnoreCase(TAX_TOTAL_OTR_ID)) {
                    TaxSubtotalType taxSubtotal = new TaxSubtotalType();
                    ...
                    if (!TAX_TOTAL_GRT_ID.equalsIgnoreCase(tributo)) {
                    ...
                    idTaxCategory.setSchemeAgen cyName(IUBLConfig.SCHEME_AGENCY_NAME_UNECE);
                    idTaxCategory.setSchemeID("UN/ECE 5305");
                    idTaxCategory.setSchemeName("Tax Category Identifier");
                    taxCategory.setID(idTaxCategory);
                    ...
                    if (tributo.equalsIgnoreCase(TAX_TOTAL_IGV_ID) || tributo.equalsIgnoreCase(TAX_TOTAL_EXP_ID) || tributo.equalsIgnoreCase(TAX_TOTAL_EXO_ID)
                            || tributo.equalsIgnoreCase(TAX_TOTAL_INA_ID) || tributo.equalsIgnoreCase(TAX_TOTAL_GRT_ID)) {
                    ...
                    } else if (tributo.equalsIgnoreCase(TAX_TOTAL_ISC_ID)) {
    ...
    protected ItemType getItemForLineRest(TransaccionLineas transaccionLineas) {//String descriptionValue, String sellersItemIdentificationIDValue) {
        ...
        if (transaccionLineas.getSubPartida() != null && !transaccionLineas.getSubPartida().isEmpty())
            item.getAdditionalItemProperty().add(getAdditionalItemPropertyRest(INDICATOR_SUBPART, "7020", transaccionLineas.getSubPartida()));
        //</cac:Item><cac:AdditionalItemProperty>
        if (transaccionLineas.getIndicadorBien() != null && !transaccionLineas.getIndicadorBien().isEmpty())
            item.getAdditionalItemProperty().add(getAdditionalItemPropertyRest(INDICATOR_REGULADO_SUNAT, "7022", transaccionLineas.getIndicadorBien()));
        //</cac:Item><cac:AdditionalItemProperty>
        if (transaccionLineas.getNumeracion() != null && !transaccionLineas.getNumeracion().isEmpty())
            item.getAdditionalItemProperty().add(getAdditionalItemPropertyRest(INDICATOR_NUMERACION_DAM_DS, "7021", transaccionLineas.getNumeracion()));
        //</cac:Item><cac:AdditionalItemProperty>
        if (transaccionLineas.getNumeroSerie() != null && !transaccionLineas.getNumeroSerie().isEmpty())
            item.getAdditionalItemProperty().add(getAdditionalItemPropertyRest(INDICATOR_SERIE_DAM_DS, "7023", transaccionLineas.getNumeroSerie()));
    ...
    protected ItemPropertyType getAdditionalItemPropertyRest(String name, String valueNameCode, String value) {//TransaccionLineas transaccionLineas){
        ...
        nameCode.setListAgencyName(SCHEME_AGENCY_NAME_PE_SUNAT);
        nameCode.setListName(PROPERTIES_NAME_ITEM);
        nameCode.setListURI(URI_CATALOG_55);
