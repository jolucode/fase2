...
public class UBLDocumentHandler20 {
    ...
    public RetentionType generateRetentionType(Transaccion transaction, String signerName) throws UBLDocumentException {
        ...
        try {
            ...
            retentionType.getSunatRetentionDocumentReference().addAll(generateDocumentReferenceV2(transaction.getTransaccionComprobantePagoList()));
    ...
    public PerceptionType generatePerceptionType(Transaccion transaction, String signerName) throws UBLDocumentException {
        ...
        try {
            ...
            perceptionType.getSunatPerceptionDocumentReference().addAll(generateDocumentReference(transaction.getTransaccionComprobantePagoList()));
    ...
    public InvoiceType generateInvoiceType(Transaccion transaction, String signerName) throws UBLDocumentException {
        ...
        try {
            ...
            invoiceType.getInvoiceLine().addAll(getAllInvoiceLines(transaction.getTransaccionLineasList(), transaction.getDOCMONCodigo()));
    ...
    public DespatchAdviceType generateDespatchAdviceType(Transaccion transaction, String signerName) throws UBLDocumentException {
        ...
        try {
            ...
            for (int i = 0; i < transaction.getTransaccionLineasList().size(); i++) {
                DespatchLineType despatchLineType = new DespatchLineType();
                IDType iDType = new IDType();
                iDType.setValue(String.valueOf(transaction.getTransaccionLineasList().get(i).getTransaccionLineasPK().getNroOrden()));
                ...
                lineIDType.setValue(String.valueOf(transaction.getTransaccionLineasList().get(i).getTransaccionLineasPK().getNroOrden()));
                orderLineReferenceType.setLineID(lineIDType);

                DeliveredQuantityType deliveredQuantityType = new DeliveredQuantityType();
                deliveredQuantityType.setUnitCode(transaction.getTransaccionLineasList().get(i).getUnidad());
                deliveredQuantityType.setValue(transaction.getTransaccionLineasList().get(i).getCantidad().setScale(0, RoundingMode.HALF_UP));

                ItemType itemType = new ItemType();
                NameType nameType = new NameType();
                nameType.setValue(transaction.getTransaccionLineasList().get(i).getDescripcion());
                itemType.setName(nameType);

                ItemIdentificationType itemIdentificationType = new ItemIdentificationType();
                IDType dType = new IDType();
                dType.setValue(transaction.getTransaccionLineasList().get(i).getCodArticulo());
    ...
    public InvoiceType generateBoletaType(Transaccion transaction, String signerName) throws UBLDocumentException {
        ...
        try {
            ...
            /* Agregar items <Invoice><cac:InvoiceLine> */
            boletaType.getInvoiceLine().addAll(getAllBoletaLines(transaction.getTransaccionLineasList(), transaction.getDOCMONCodigo()));
        } catch (UBLDocumentException e) {
    ...
    public CreditNoteType generateCreditNoteType(Transaccion transaction, String signerName) throws UBLDocumentException {
        ...
        try {
            ...
            UBLExtensionsType ublExtensions = new UBLExtensionsType();
            {
                 ...
                 * ><sac:AdditionalInformation>
                 */
                ublExtensions.getUBLExtension().add(getUBLExtensionTotalAndPropertyForNCND(transaction.getTransaccionTotalesList(), transaction.getTransaccionPropiedadesList(), transaction));
            ...
            creditNoteType.getCreditNoteLine().addAll(getAllCreditNoteLines(transaction.getTransaccionLineasList(), transaction.getDOCMONCodigo()));
        } catch (UBLDocumentException e) {
    ...
    public DebitNoteType generateDebitNoteType(Transaccion transaction, String signerName) throws UBLDocumentException {
        ...
        try {
            ...
            debitNoteType.getDebitNoteLine().addAll(getAllDebitNoteLines(transaction.getTransaccionLineasList(), transaction.getDOCMONCodigo()));
        } catch (UBLDocumentException e) {
...
//extensionContent.setAny((org.w3c.dom.Element) getExtensionContentNode(additionalInformation));
    ...
    private UBLExtensionType getUBLExtensionTotalAndPropertyForNCND(List<TransaccionTotales> transactionTotalList, List<TransaccionPropiedades> transactionPropertyList, Transaccion transaccion) throws UBLDocumentException {
        if (logger.isDebugEnabled()) {
            logger.debug("+getUBLExtensionTotalAndProperty() [" + this.identifier + "] transactionTotalList: " + transactionTotalList + " transactionPropertyList: " + transactionPropertyList + " sunatTransactionID: " + transaccion.getSUNATTransact());
        }
        UBLExtensionType ublExtension = null;
        try {
            ublExtension = new UBLExtensionType();
            ...
            if (null == transactionTotalList) {
                ...
                if (StringUtils.isNotBlank(transaccion.getSUNATTransact())) {
                    if (logger.isDebugEnabled()) {
                        logger.debug("getUBLExtensionTotalAndProperty() [" + this.identifier + "] Existe valor SUNATTransaction.");
                    }
                    SUNATTransactionType sunatTransaction = new SUNATTransactionType();
                    IDType id = new IDType();
                    id.setValue(transaccion.getSUNATTransact().trim());
                    sunatTransaction.setID(id);
                    additionalInformation.setSUNATTransaction(sunatTransaction);
                }
        ...
        if (lstAnticipo != null) {
            ...
            for (int i = 0; i < lstAnticipo.size(); i++) {
                ...
                try {
                    ...
                    /** Harol tag PaidAmount se quita el redondeo*/
                    PaidAmountType paidAmount = new PaidAmountType();
                    paidAmount.setValue(lstAnticipo.get(i).getAnticipoMonto().setScale(2, RoundingMode.HALF_UP));
