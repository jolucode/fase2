...
import org.ventura.cpe.loader.VSCPELoader;
...
public class Programa {
    ...
    public static void start() {
        Thread T = new Thread(() -> {
            ...
            LoggerTrans.createCDLogger("Main_Resumen");
            LoggerTrans.createCDLogger("Main_Loader");
            LoggerTrans.createCDLogger("Main_PublicWS");

            if (!Loader.CargarParametros()) {
                MENSAJE = "ERR: No se pudieron cargar los parametros de configuracion.";
                ESTADO = ESTADO_DETENIDO;
                return;
            }
            if (!Interfaz.CargarParametros()) {
                MENSAJE = "ERR: No se pudieron cargar los parametros de configuracion.";
                ESTADO = ESTADO_DETENIDO;
                return;
            }
            ...
            if (!InstanciaUnica(PuertoVSCPE.PuertoAbierto_Configurador)) {
                JOptionPane.showMessageDialog(null, "Existe otra instancia de la aplicación ejectandose.");
                System.exit(0);
                return;
            } else {
                LoggerTrans.getCDMainLogger().log(Level.CONFIG, "Puerto encontrado {0}", PuertoVSCPE.PuertoAbierto_Configurador);
            }
            ...
            if (mensaje.startsWith("ERR:")) {
                ...
                if (DocumentoBL.listSociedades != null && DocumentoBL.listSociedades.size() > 0) {
                    ...
                    for (ICompany sociedad : DocumentoBL.listSociedades) {
                        LoggerTrans.getCDMainLogger().log(Level.INFO, "Se conecto a :: {0}", sociedad.getCompanyName());
                    }
                }
    ...
    public static void stop() {
        Thread T = new Thread(() -> {
            ...
            try {
                ...
                LoggerTrans.cerrarLogger("Main_Resumen");
                LoggerTrans.cerrarLogger("Main_Loader");
                LoggerTrans.cerrarLogger("Main_Main");
                LoggerTrans.cerrarLogger("Main_PublicWS");
            }
        });
        T.start();
    }
