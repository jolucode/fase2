package ventura.soluciones.signer.utils;
...
public class CertificateUtils
{
	 ...
	 * 		La ruta en disco del certificado digital.
	 * @return
	 * 		Retorna el certificado digital en bytes.
	...
	{
		if (logger.isDebugEnabled()) {logger.debug("+getCertificateInBytes() certificatePath: " + certificatePath);}
...
//			throw e;
		...
		if (logger.isDebugEnabled()) {logger.debug("-getCertificateInBytes()");}
	...
	{
		if (logger.isDebugEnabled()) {logger.debug("+getCertificateInBytes() certificatePath: " + filePath);}
		byte[] certificateBytes = null;
		ByteArrayOutputStream baos = null;
		FileInputStream fis = null;
...
//			throw e;
		...
		if (logger.isDebugEnabled()) {logger.debug("-getFile()");}
		return certificateBytes;
	} //getCertificateInBytes
	 ...
	 * @return
	 * 		Retorna un valor booleano que indica si el proceso fue satisfactorio o no.
	 */
	...
	{
		if (logger.isDebugEnabled()) {logger.debug("+checkDigitalCertificate()");}
		boolean flag = false;
		...
		try
		{
			...
			if (logger.isDebugEnabled()) {logger.debug("checkDigitalCertificate() alias: " + alias);}
	...
	public static boolean checkDigitalCertificateV2(byte[] certificate, String certificatePwd, String keystoreProvider, String keystoreType) 
			throws SignerDocumentException
	{
		long startTime = System.currentTimeMillis();
		if (logger.isDebugEnabled()) {logger.debug("+checkDigitalCertificateV2()");}
		boolean flag = false;
		...
		try
		{
			if (null != certificate)
			{
				...
				String alias = getAliasFromCertificate(keystore);
				if (logger.isDebugEnabled()) {logger.debug("checkDigitalCertificateV2() alias: " + alias);}
		...
		if (logger.isDebugEnabled()) {logger.debug("-checkDigitalCertificateV2() TIME: " + (System.currentTimeMillis() - startTime) + " ms");}
		return flag;
	} //checkDigitalCertificate
