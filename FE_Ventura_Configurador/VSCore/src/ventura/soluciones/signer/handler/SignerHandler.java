package ventura.soluciones.signer.handler;
...
public class SignerHandler
{
	 ...
	 * 
	 * @param identifier
	 * 		Identificador del objeto SignerHandler.
	 */
	 ...
	 * 
	 * @return
	 * 		Retorna una nueva instancia de la clase SignerHandler.
	 */
	 ...
	 * 
	 * @param certificate
	 * 		El certificado digital en bytes.
	 * @param certificatePwd
	 * 		La contrasenia del certificado digital.
	 * @param signerName
	 * 		El nombre del firmante del certificado digital.
	...
	public void setConfiguration(byte[] certificate, String certificatePwd, String keystoreType, String keystoreProvider, String signerName)
			throws SignerDocumentException
	{
		if (logger.isDebugEnabled()) {logger.debug("+setConfiguration()" + (null != this.identifier ? " [" + this.identifier + "]" : ""));}
	 ...
	 * 
	 * @param documentPath
	 * 		La ruta del documento UBL sin firmar.
	 * @param docUUID
	 * 		UUID del documento correspondiente al registro del objeto Homologationdocument.
	 * @return
	 * 		Retorna un objeto File con la ruta del documento UBL firmado.
	 * @throws SignerDocumentException
	 * @throws FileNotFoundException
	 */
	@SuppressWarnings("unused")
	public File signDocument(String documentPath, String docUUID) throws SignerDocumentException, FileNotFoundException
	{
		if (logger.isDebugEnabled()) {logger.debug("+signDocument()" + (null != this.identifier ? " [" + this.identifier + "]" : "") + " [" + docUUID + "]");}
		...
		try
		{
			...
			signContext.putNamespacePrefix(javax.xml.crypto.dsig.XMLSignature.XMLNS, ISignerConfig.SIGN_CONTEXT_NAMESPACE_PREFIX);
			...
			Reference reference = signatureFactory.newReference("", signatureFactory.newDigestMethod(javax.xml.crypto.dsig.DigestMethod.SHA1, 
					null), java.util.Collections.singletonList(signatureFactory.newTransform(javax.xml.crypto.dsig.Transform.ENVELOPED, 
			...
			if (logger.isDebugEnabled()) {logger.debug("signDocument()" + (null != this.identifier ? " [" + this.identifier + "]" : "") + " [" + docUUID + "] Documento UBL firmado correctamente.");}
		}
		catch (Exception e)
		{
	...
	{
		if (logger.isDebugEnabled()) {logger.debug("+loadDigitalCertificate()" + (null != this.identifier ? " [" + this.identifier + "]" : ""));}
		try
		{
	 ...
	 * 		El objeto KeyStore.
	 * @return
	 * 		Retorna el ALIAS del certificado digital.
	 * @throws SignerDocumentException
	 */
	private String getAliasFromCertificate(KeyStore keyStore) throws SignerDocumentException
	{
		...
		try
		{
			...
			{
				alias = (String)aliasEnumeration.nextElement();
	 ...
	 * 
	 * @param document
	 * 		El objeto Document que contien el archivo XML.
	 * @return
	 * 		Retorna el nodo para firmar.
	 * @throws Exception
	 */
	private Node getElementPositionToSign(Document document, String docUUID) throws Exception
	{
		if (logger.isDebugEnabled()) {logger.debug("+-getElementPositionToSign()" + (null != this.identifier ? " [" + this.identifier + "]" : "") + " [" + docUUID + "]");}
		Node node = null;
		...
		if (null != nodeList && 0 < nodeList.getLength())
		{
			...
			for (int i=0; i<childNodes.getLength(); ++i)
			{
				...
				{
					node = (Node) childNodes.item(i); break;
				}
			}
		}
		else
