package org.ventura.cpe.core.signer.handler;

import org.apache.commons.lang3.exception.ExceptionUtils;
import org.apache.log4j.Logger;
import org.ventura.cpe.core.config.ISignerConfig;
import org.ventura.cpe.core.exception.IVenturaError;
import org.ventura.cpe.core.exception.SignerDocumentException;
...
public class SignerHandler {
     ...
     */
    public void setConfiguration(byte[] certificate, String certificatePwd, String keystoreType, String keystoreProvider, String signerName) throws SignerDocumentException {
    ...
    public File signDocument(String documentPath, String docUUID) throws SignerDocumentException {
        ...
        try {
            ...
            Reference reference = signatureFactory.newReference("", signatureFactory.newDigestMethod(javax.xml.crypto.dsig.DigestMethod.SHA1, null), Collections.singletonList(signatureFactory.newTransform(javax.xml.crypto.dsig.Transform.ENVELOPED, (TransformParameterSpec) null)), null, null);

            SignedInfo signedInfo = signatureFactory.newSignedInfo(signatureFactory.newCanonicalizationMethod(javax.xml.crypto.dsig.CanonicalizationMethod.INCLUSIVE, (C14NMethodParameterSpec) null), signatureFactory.newSignatureMethod(javax.xml.crypto.dsig.SignatureMethod.RSA_SHA1, null), Collections.singletonList(reference));
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("signDocument()" + (null != this.identifier ? " [" + this.identifier + "]" : "") + " [" + docUUID + "] Documento UBL firmado correctamente.");
            }
        } catch (Exception e) {
    ...
    private void loadDigitalCertificate() throws SignerDocumentException {
        ...
        try {
            ...
            PrivateKeyEntry privateKeyEntry = (PrivateKeyEntry) keyStore.getEntry(alias, new KeyStore.PasswordProtection(password.toCharArray()));
