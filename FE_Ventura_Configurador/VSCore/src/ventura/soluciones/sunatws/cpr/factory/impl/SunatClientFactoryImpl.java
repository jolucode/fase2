package ventura.soluciones.sunatws.cpr.factory.impl;
...
import ventura.soluciones.sunatws.cpr.client.production.ProductionCPEClient;
import ventura.soluciones.sunatws.cpr.client.test.TestCPEClient;
import ventura.soluciones.sunatws.cpr.config.ISunatConfig;
import ventura.soluciones.sunatws.cpr.factory.ISunatClient;
import ventura.soluciones.sunatws.cpr.factory.SunatClientFactory;
...
public final class SunatClientFactoryImpl extends SunatClientFactory
{
	...
	public ISunatClient getSunatClient(String clientName,String documentType) throws ConfigurationException
	{
		...
		if (clientName.equalsIgnoreCase(ISunatConfig.TEST_CLIENT))
		{
			...
			if (logger.isInfoEnabled()) {logger.info("getSunatClient() Create a TEST_CLIENT para PERCEPCIONES Y RETENCIONES.");}
			
		}
		else if (clientName.equalsIgnoreCase(ISunatConfig.PRODUCTION_CLIENT))
		{
			sunatClient = new ProductionCPEClient();
			if (logger.isInfoEnabled()) {logger.info("getSunatClient() Create a PRODUCTION_CLIENT para PERCEPCIONES Y RETENCIONES.");}
		}
		else
		{
