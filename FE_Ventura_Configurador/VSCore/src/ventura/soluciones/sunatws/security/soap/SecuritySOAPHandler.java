package ventura.soluciones.sunatws.security.soap;

import org.apache.commons.lang3.exception.ExceptionUtils;
import org.apache.log4j.Logger;
import ventura.soluciones.sunatws.config.ISunatConfig;
...
public class SecuritySOAPHandler implements SOAPHandler<SOAPMessageContext>
{
	 ...
	 * 
	 * @param consumer
	 * 		El objeto consumidor que contiene los datos del emisor electronico.
	 * @param printOption
	...
	public boolean handleMessage(SOAPMessageContext context)
	{
		...
		if (outboundProperty.booleanValue())
		{
			...
			try
			{
				...
				logger.info("handleMessage() envelope: " + envelope.getHeader());
				logger.info("handleMessage() envelope: " + envelope.getTextContent());
				...
				SOAPElement security = header.addChildElement(ISunatConfig.WS_SECURITY_HEADER_PARENT_NAME, 
						ISunatConfig.WS_SECURITY_HEADER_PARENT_BASE_PFX, ISunatConfig.WS_SECURITY_HEADER_PARENT_VALUE);
				
				SOAPElement usernameToken = security.addChildElement(ISunatConfig.WS_SECURITY_SUB_UNTOKEN_NAME, 
						ISunatConfig.WS_SECURITY_HEADER_PARENT_BASE_PFX);
				usernameToken.addAttribute(new QName(ISunatConfig.WS_SECURITY_SUB_UNTOKEN_PFX), 
						ISunatConfig.WS_SECURITY_SUB_UNTOKEN_VALUE);
						
				SOAPElement username = usernameToken.addChildElement(ISunatConfig.WS_SECURITY_PARAM_USERNAME, 
						ISunatConfig.WS_SECURITY_HEADER_PARENT_BASE_PFX);	
				username.addTextNode(this.consumer.getRuc() + this.consumer.getUsername());
				
				SOAPElement password = usernameToken.addChildElement(ISunatConfig.WS_SECURITY_PARAM_PASSWORD, 
						ISunatConfig.WS_SECURITY_HEADER_PARENT_BASE_PFX);
				password.setAttribute(ISunatConfig.WS_SECURITY_COMMON_ATTRIBUTE_TYPE,
						ISunatConfig.WS_SECURITY_COMMON_ATRIBUTE_PWD_VALUE);
				...
				if (printOption)
				{
					...
					if (logger.isInfoEnabled()) {logger.info("handleMessage() [REQUEST] Mensaje SOAP: \n" + baos.toString());}
				}
			}
			catch (Exception e)
			{
		...
		else
		{
			try
			{
				if (printOption)
				{
					...
					ByteArrayOutputStream baos = new ByteArrayOutputStream();
					message.writeTo(baos);
					if (logger.isInfoEnabled()) {logger.info("handleMessage() [RESPONSE] Mensaje SOAP: \n" + baos.toString());}
				}
			}
			catch (Exception e)
			{
