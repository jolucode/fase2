...
import org.ventura.soluciones.commons.handler.basic.UBLBasicHandler;
...
public class UBLDocumentHandler extends UBLBasicHandler {
    ...
    /** Harol 27-06-2024 Texto Amplio, Tag Note*/
        ...
        try {
            ...
            /*if (null == transactionTotalList) {
                ...
                if (StringUtils.isNotBlank(sunatTransactionID)) {
                    ...
                    if(!sunatTransactionID.trim().isEmpty()){
            ...
            if (transaccion.getTransaccionContractdocrefList() != null && !transaccion.getTransaccionContractdocrefList().isEmpty()) {
                ...
                Optional<TransaccionContractdocref> listTran =
                        transaccion.getTransaccionContractdocrefList().stream().filter(docRefer -> "incoterms".equalsIgnoreCase(docRefer.getUsuariocampos().getNombre())).findFirst();
                ...
                if (objecto != null && !objecto.getValor().trim().isEmpty()) {
            ...
            ExtensionContentType extensionContent = new ExtensionContentType();
            if(additionalInformation.getAdditionalProperty().size() > 0 || additionalInformation.getSUNATTransaction() != null){
                extensionContent.setAny((org.w3c.dom.Element) getExtensionContentNode(additionalInformation));
            }
            if(!textoAmplio.trim().isEmpty() && !textoAmplio.equalsIgnoreCase("")){
                NoteType noteType = new NoteType();
                noteType.setValue(textoAmplio);
                //ublExtension.setNote(noteType);
    ...
    private Boolean getCampoUsuario(Transaccion transaccion){
        for (int i = 0; i < transaccion.getTransaccionContractdocrefList().size(); i++) {
            if ("texto_amplio".equals(transaccion.getTransaccionContractdocrefList().get(i).getUsuariocampos().getNombre()) || "incoterms".equals(transaccion.getTransaccionContractdocrefList().get(i).getUsuariocampos().getNombre())) {
                return true;
            }
        }
        return false;
    ...
    public InvoiceType generateInvoiceType(Transaccion transaction, String signerName) throws UBLDocumentException {
        ...
        try {
            ...
            Boolean existeCampo = getCampoUsuario(transaction);
            if(existeCampo){
...
//            }
 ...
 /*if (logger.isDebugEnabled()) {
            ...
            if (null != transaction.getANTICIPOMonto() && transaction.getANTICIPOMonto().compareTo(BigDecimal.ZERO) > 0 && null != transaction.getTransaccionAnticipoList() && 0 < transaction.getTransaccionAnticipoList().size()) {
                ...
                invoiceType.getAllowanceCharge().add(getAllowanceCharge(transaction.getDOCImporteTotal(), transaction.getANTICIPOMonto(), false, transaction.getDOCPorDescuento(), transaction.getDOCDescuento(), transaction.getDOCImporteTotal(), transaction.getDOCMONCodigo(), "04", transaction.getMontoRetencion(), transaction.getDOCMontoTotal()));

            }
            /*
             * Agregar DESCUENTO GLOBAL
             */
            if (transaction.getDOCDescuento().compareTo(BigDecimal.ZERO) > 0) {
                if (logger.isInfoEnabled()) {
                ...
                //invoiceType.getAllowanceCharge().add(getAllowanceCharge(transaction.getDOCImporteTotal(), transaction.getANTICIPOMonto(), false, transaction.getDOCPorDescuento(), transaction.getDOCDescuento(), transaction.getDOCImporteTotal(), transaction.getDOCMONCodigo(), "02", transaction.getMontoRetencion(), transaction.getDOCMontoTotal()));
                //invoiceType.getAllowanceCharge().add(getAllowanceCharge(transaction.getDOCImporteTotal(), transaction.getANTICIPOMonto(), false, transaction.getDOCPorDescuento(), transaction.getDOCDescuento(), transaction.getDOCImporteTotal(), transaction.getDOCMONCodigo(), "03", transaction.getMontoRetencion(), transaction.getDOCMontoTotal()));
                if(transaction.getFEFormSAP().contains("exportacion") || transaction.getTipoOperacionSunat().startsWith("02")){
                    invoiceType.getAllowanceCharge().add(getAllowanceCharge(transaction.getDOCImporteTotal(), transaction.getANTICIPOMonto(), false, transaction.getDOCPorDescuento(), transaction.getDOCDescuento(), transaction.getDOCImporteTotal(), transaction.getDOCMONCodigo(), "03", transaction.getMontoRetencion(), transaction.getDOCMontoTotal()));
                }else {
                    invoiceType.getAllowanceCharge().add(getAllowanceCharge(transaction.getDOCImporteTotal(), transaction.getANTICIPOMonto(), false, transaction.getDOCPorDescuento(), transaction.getDOCDescuento(), transaction.getDOCImporteTotal(), transaction.getDOCMONCodigo(), "02", transaction.getMontoRetencion(), transaction.getDOCMontoTotal()));
                }
            ...
            if (transaction.getMontoRetencion() != null) {
                if (transaction.getMontoRetencion().compareTo(BigDecimal.ZERO) > 0) {
                    invoiceType.getAllowanceCharge().add(getAllowanceCharge(transaction.getDOCImporteTotal(), transaction.getANTICIPOMonto(), false, transaction.getDOCPorDescuento(), transaction.getDOCDescuento(), transaction.getDOCImporteTotal(), transaction.getDOCMONCodigo(), "62", transaction.getMontoRetencion(), transaction.getDOCMontoTotal()));
                }
            ...
            invoiceType.getTaxTotal().add(getTaxTotalV21(transaction, new ArrayList<>(transaccionImpuestosList), transaction.getDOCImpuestoTotal(), transaction.getDOCMONCodigo()));
            if (logger.isInfoEnabled()) {
            ...
            for (TransaccionLineas transaccionLinea : transaccionLineas) {
                if (Objects.equals(IUBLConfig.ALTERNATIVE_CONDICION_UNIT_PRICE, transaccionLinea.getPrecioRefCodigo()) ||
                        Objects.equals(IUBLConfig.ALTERNATIVE_CONDICION_REFERENCE_VALUE, transaccionLinea.getPrecioRefCodigo()) ) {
            ...
            BigDecimal lineExtensionAmount = transaction.getDOCImporteTotal();
            ...
            if (transaction.getDOCDescuento().compareTo(BigDecimal.ZERO) > 0){
                taxInclusiveAmount= taxInclusiveAmount.add(transaction.getDOCDescuento());
                lineExtensionAmount = lineExtensionAmount.add(transaction.getDOCDescuento());
            }
//            Optional<TransaccionImpuestos> impuestoOptional = transaccionImpuestosList.stream().filter(impuesto -> IUBLConfig.TAX_TOTAL_BPT_NAME.equalsIgnoreCase(impuesto.getNombre())).findAny();
//            if (impuestoOptional.isPresent()) {
//                TransaccionImpuestos impuesto = impuestoOptional.get();
//                taxInclusiveAmount = taxInclusiveAmount.add(impuesto.getValorVenta());
//                payableAmount = payableAmount.add(impuesto.getValorVenta());
//            }
            BigDecimal docDescuentoTotal = transaction.getDOCDescuentoTotal();
            System.out.println("*************************************************************************");
            invoiceType.setLegalMonetaryTotal(getMonetaryTotal(transaction, lineExtensionAmount, taxInclusiveAmount, noContainsFreeItem, otrosCargosValue, transaction.getANTICIPOMonto(), payableAmount, docDescuentoTotal,
                    transaction.getDOCMONCodigo(), true));
            if (logger.isInfoEnabled()) {
            ...
            invoiceType.getInvoiceLine().addAll(getAllInvoiceLines(transaction, transaccionLineas, transaction.getTransaccionPropiedadesList(), transaction.getDOCMONCodigo()));
            if (logger.isInfoEnabled()) {
    ...
    public InvoiceType generateBoletaType(Transaccion transaction, String signerName) throws UBLDocumentException {
        ...
        try {
            ...
            /** * Harol 10-08-2023 Percepciones*/
            Set<TransaccionImpuestos> transaccionImpuestosList = new HashSet<>(transaction.getTransaccionImpuestosList());
            boletaType.getTaxTotal().add(getTaxTotalV21(transaction, new ArrayList<>(transaccionImpuestosList), transaction.getDOCImpuestoTotal(), transaction.getDOCMONCodigo()));
            if (logger.isInfoEnabled()) {
            ...
            BigDecimal docDescuentoTotal = transaction.getDOCDescuentoTotal();
            boletaType.setLegalMonetaryTotal(getMonetaryTotal(transaction, transaction.getDOCImporte(), transaction.getDOCSinPercepcion(), noContainsFreeItem, transaction.getDOCOtrosCargos(), transaction.getANTICIPOMonto(),
                    transaction.getDOCMontoTotal(), docDescuentoTotal, transaction.getDOCMONCodigo(), true));
            if (logger.isInfoEnabled()) {
            ...
            /* Agregar <Invoice><cac:InvoiceLine> */
            boletaType.getInvoiceLine().addAll(getAllBoletaLines(transaction, transaccionLineas, transaction.getTransaccionPropiedadesList(), transaction.getDOCMONCodigo()));
            if (logger.isInfoEnabled()) {
...
//            }
            ...
            if (transaction.getDOCSerie().contains("F")) { //Aseguramos que sea NC solo para FACTURAS
            ...
            creditNoteType.setLegalMonetaryTotal(getMonetaryTotal(transaction, transaction.getDOCImporte(), transaction.getDOCSinPercepcion(), false, transaction.getDOCOtrosCargos(), null,
                    transaction.getDOCMontoTotal(), docDescuentoTotal, transaction.getDOCMONCodigo(), false));
            if (logger.isInfoEnabled()) {
...
//            creditNoteType.getLegalMonetaryTotal().setPayableRoundingAmount(payableRoundingAmount);
            ...
            creditNoteType.getCreditNoteLine().addAll(getAllCreditNoteLines(transaction, transaction.getTransaccionLineasList(), transaction.getTransaccionPropiedadesList(), transaction.getDOCMONCodigo()));
            if (logger.isInfoEnabled()) {
    ...
    public DebitNoteType generateDebitNoteType(Transaccion transaction, String signerName) throws UBLDocumentException {
        ...
        try {
            ...
            debitNoteType.setRequestedMonetaryTotal(getMonetaryTotal(transaction, transaction.getDOCImporte(), transaction.getDOCSinPercepcion(), false, transaction.getDOCOtrosCargos(),
                    null, transaction.getDOCMontoTotal(), docDescuentoTotal, transaction.getDOCMONCodigo(), false));
            if (logger.isInfoEnabled()) {
...
//            debitNoteType.getRequestedMonetaryTotal().setPayableRoundingAmount(payableRoundingAmount);
            ...
            debitNoteType.getDebitNoteLine().addAll(getAllDebitNoteLines(transaction, transaction.getTransaccionLineasList(), transaction.getTransaccionPropiedadesList(), transaction.getDOCMONCodigo()));
            if (logger.isInfoEnabled()) {
    ...
    public DespatchAdviceType generateDespatchAdviceType(Transaccion transaction, String signerName) throws UBLDocumentException {
        ...
        try {
            ...
            if (transaction.getTransaccionGuiaRemision().getCodigoMotivo().equals("02")) {/*Motivo de Venta: Compra - 02*/
                despatchAdviceType.setDeliveryCustomerParty(getDeliveryCustomerParty(transaction.getDocIdentidadNro(), transaction.getDocIdentidadTipo(), transaction.getRazonSocial()));
            } else {
            ...
            if (transaction.getTransaccionGuiaRemision().getCodigoMotivo().equals("02") || transaction.getTransaccionGuiaRemision().getCodigoMotivo().equals("07") ||
                ...
                despatchAdviceType.setSellerSupplierParty(getSellerSupplierParty(transaction.getSNDocIdentidadNro(), transaction.getSNDocIdentidadTipo(), transaction.getSNRazonSocial()));
            }
            ...
            if (transaction.getTransaccionGuiaRemision().getCodigoMotivo().equals("03") || transaction.getTransaccionGuiaRemision().getCodigoMotivo().equals("13"))
                despatchAdviceType.setBuyerCustomerParty(getBuyerCustomerParty(transaction.getSNDocIdentidadNro(), transaction.getSNDocIdentidadTipo(), transaction.getSNRazonSocial()));
            /***/
    ...
    public List<NoteType> getDespatchNoteList(Transaccion transaccion) {
    ...
    public DespatchAdviceType generateCarrierDespatchAdviceType(Transaccion transaction, String signerName) throws UBLDocumentException {
        if (logger.isDebugEnabled()) {
        ...
        try {
            ...
            /* Agregar <DespatchAdvice><cbc:ID> */
            if (logger.isInfoEnabled()) {
                logger.info("generateTransportDespatchAdviceType() [" + this.identifier + "] Agregando DOC_Id: " + transaction.getDOCId());
            }
            despatchAdviceType.setID(getID(transaction.getDOCId()));
            /* Agregar <DespatchAdvice><cbc:IssueDate> */
            despatchAdviceType.setIssueDate(getIssueDate(transaction.getDOCFechaEmision()));
            ...
            List<TransaccionDocrefers> transaccionDocrefers = transaction.getTransaccionDocrefersList();
            // Realiza operaciones con cada elemento 'transaccion_docrefers'
            despatchAdviceType.getAdditionalDocumentReference().addAll(getAdditionalDocumentReference(transaction.getTransaccionGuiaRemision(), transaccionDocrefers));

            /* Agregar <DespatchAdvice><cac:DespatchSupplierParty> */
            despatchAdviceType.setDespatchSupplierParty(getDespatchSupplierParty(transaction.getDocIdentidadNro(), transaction.getDocIdentidadTipo(), transaction.getRazonSocial()));

            /* Agregar <DespatchAdvice><cac:DeliveryCustomerParty> */
            despatchAdviceType.setDeliveryCustomerParty(getDeliveryCustomerParty(transaction.getTransaccionGuiaRemision().getGRT_DocumentoDestinatario(), transaction.getTransaccionGuiaRemision().getGRT_TipoDocDestinatario(), transaction.getTransaccionGuiaRemision().getGRT_NombreRazonDestinatario()));

            /* Agregar <DespatchAdvice><cac:OriginatorCustomerParty>*/
            despatchAdviceType.setOriginatorCustomerParty(getOriginatorCustomerParty(transaction.getSNDocIdentidadNro(), transaction.getSNDocIdentidadTipo(), transaction.getSNRazonSocial()));

            /* Agregar <DespatchAdvice><cac:Shipment> */
            despatchAdviceType.setShipment(getShipmentCarrier(transaction.getTransaccionGuiaRemision(), transaction));
    ...
    protected List<DocumentReferenceType> getAdditionalDocumentReference(TransaccionGuiaRemision guia, List<TransaccionDocrefers> transaccionDocrefersList) {

        List<DocumentReferenceType> documentReferenceTypesList = new ArrayList<DocumentReferenceType>(transaccionDocrefersList.size());

        for(TransaccionDocrefers docref : transaccionDocrefersList) {
            DocumentReferenceType documentReferenceType = new DocumentReferenceType();
    ...
    private DocumentReferenceType insertarDocRefers(TransaccionGuiaRemision guia, Transaccion transaccion) {
        ...
        PartyIdentificationType partyIdentificationParty = new PartyIdentificationType();
        IDType idIdentification = new IDType();
        idIdentification.setSchemeID(transaccion.getDocIdentidadTipo());
        idIdentification.setSchemeAgencyName(IUBLConfig.SCHEME_AGENCY_NAME_PE_SUNAT);
        idIdentification.setSchemeURI(IUBLConfig.URI_CATALOG_06);
        idIdentification.setValue(transaccion.getDocIdentidadNro());
        partyIdentificationParty.setID(idIdentification);
        issuerParty.getPartyIdentification().add(partyIdentificationParty);
        documentReferenceType.setIssuerParty(issuerParty);
    ...
    private DocumentReferenceType generateDAMNumber(String docCondPago) {
        DocumentReferenceType documentReferenceType = new DocumentReferenceType();
        IDType idType = new IDType();
        idType.setValue(docCondPago);
        documentReferenceType.setID(idType);
        DocumentTypeCodeType documentTypeCodeType = new DocumentTypeCodeType();
        documentTypeCodeType.setValue("01");
...
//                            transaccionLinea.getDSCTOMonto(),transaccionLinea.getDSCTOMonto(), currencyCode));
    ...
    public List<SUNATRetentionDocumentReferenceType> getAllSUNATRetentionDocumentReferences(List<TransaccionComprobantePago> transaccionComprobantePagoList) throws UBLDocumentException {
        ...
        try {
            for (TransaccionComprobantePago transaccionComprobantePago : transaccionComprobantePagoList) {
                /** Harol 24-04-2024 Exclusion de Tag SUNATRetentionDocumentReference Payment y SUNATRetentionInformation sí incluye NC*/
                if(transaccionComprobantePago.getDOCTipo().equalsIgnoreCase("07")) {
    ...
    private PaymentTermsType createPaymentTerms(Transaccion transaccion) {
        ...
        BigDecimal importeTotal = transaccion.getDOCSinPercepcion();
    ...
    private AllowanceChargeType createAllowanceChargeType(Transaccion transaccion) {
        ...
        multiplierFactorNumeric.setValue(transaccion.getdOCPorPercepcion().setScale(5, BigDecimal.ROUND_HALF_UP));
        ...
        baseAmount.setValue(transaccion.getDOCSinPercepcion().setScale(2, BigDecimal.ROUND_HALF_UP));
