...
import org.ventura.cpe.dto.hb.TransaccionImpuestos;
import org.ventura.cpe.dto.hb.TransaccionTotales;
...
import org.ventura.soluciones.pdfcreator.object.item.*;
...
//            throw new PDFReportException(IVenturaError.ERROR_422);
    ...
    public List<BoletaItemObject> getBoletaItems(
            List<InvoiceLineType> boletaLines) throws PDFReportException {

        if (logger.isDebugEnabled()) {
            logger.debug("+getBoletaItems() [" + this.docUUID
                    + "] boletaLines: " + boletaLines);
        }
        ...
        if (null != boletaLines && 0 < boletaLines.size()) {
            ...
            try {
                for (InvoiceLineType bLine : boletaLines) {
                    ...
                    boletaItemObj.setQuantityItem(bLine.getInvoicedQuantity()
                            .getValue());
                    // boletaItemObj.setQuantityItem(bLine.getInvoicedQuantity().getValue().setScale(IPDFCreatorConfig.DECIMAL_ITEM_QUANTITY,
                    // RoundingMode.HALF_UP).toString());
                    boletaItemObj
                            .setUnitMeasureItem((null != bLine.getNote()) ? bLine
                                    .getNote().get(0).getValue() : "");
                    boletaItemObj.setDescriptionItem(bLine.getItem()
                            .getDescription().get(0).getValue().toUpperCase());
                    boletaItemObj.setUnitValueItem(bLine.getPrice()
                            .getPriceAmount().getValue());
                    // boletaItemObj.setUnitValueItem(getCurrencyV2(bLine.getPrice().getPriceAmount().getValue(),
                    // null, IPDFCreatorConfig.PATTERN_FLOAT_DEC_3));

                    BigDecimal unitPrice = getPricingReferenceValue(bLine
                                    .getPricingReference()
                                    .getAlternativeConditionPrice(),
                            IPDFCreatorConfig.ALTERNATIVE_COND_UNIT_PRICE);
                    ...
                    boletaItemObj.setDiscountItem(getCurrency(
                            getDiscountItem(bLine.getAllowanceCharge()), null));

                    boletaItemObj.setAmountItem(getCurrency(
                            IUBLConfig.lstImporteIGV.get(contador), null));

                    itemList.add(boletaItemObj);
                    contador++;
                }
            } catch (PDFReportException e) {
                logger.error("getBoletaItems() [" + this.docUUID + "] ERROR: "
                        + e.getMessage());
                throw e;
            } catch (Exception e) {
                logger.error("getBoletaItems() [" + this.docUUID + "] ERROR: "
    ...
    protected List<CreditNoteItemObject> getCreditNoteItems(
            List<CreditNoteLineType> creditNoteLines) throws PDFReportException {
        if (logger.isDebugEnabled()) {
            logger.debug("+getCreditNoteItems() [" + this.docUUID
        ...
        if (null != creditNoteLines && 0 < creditNoteLines.size()) {
            ...
            try {
                for (CreditNoteLineType cnLine : creditNoteLines) {
                    ...
                    creditNoteItemObj.setQuantityItem(cnLine
                            .getCreditedQuantity().getValue());
                    // creditNoteItemObj.setQuantityItem(cnLine.getCreditedQuantity().getValue().setScale(IPDFCreatorConfig.DECIMAL_ITEM_QUANTITY,
                    // RoundingMode.HALF_UP).toString());
                    creditNoteItemObj.setUnitMeasureItem((null != cnLine
                            .getNote()) ? cnLine.getNote().get(0).getValue() : "");
                    creditNoteItemObj.setDescriptionItem(cnLine.getItem()
                            .getDescription().get(0).getValue().toUpperCase());
                    creditNoteItemObj.setUnitValueItem(cnLine.getPrice()
                            .getPriceAmount().getValue());
                    // creditNoteItemObj.setUnitValueItem(getCurrencyV2(cnLine.getPrice().getPriceAmount().getValue(),
                    // null, IPDFCreatorConfig.PATTERN_FLOAT_DEC_3));

                    BigDecimal unitPrice = getPricingReferenceValue(cnLine
                                    .getPricingReference()
                                    .getAlternativeConditionPrice(),
                            IPDFCreatorConfig.ALTERNATIVE_COND_UNIT_PRICE);
                    ...
                    creditNoteItemObj.setAmountItem(getCurrency(cnLine
                            .getLineExtensionAmount().getValue(), null));

                    itemList.add(creditNoteItemObj);
                }
            } catch (PDFReportException e) {
                logger.error("getCreditNoteItems() [" + this.docUUID
                        + "] ERROR: " + e.getMessage());
                throw e;
            } catch (Exception e) {
                logger.error("getCreditNoteItems() [" + this.docUUID
                        + "] ERROR: " + IVenturaError.ERROR_415.getMessage());
                throw new PDFReportException(IVenturaError.ERROR_415);
            }
        } else {
            logger.error("getCreditNoteItems() [" + this.docUUID + "] ERROR: "
                    + IVenturaError.ERROR_411.getMessage());
    ...
    protected List<DebitNoteItemObject> getDebitNoteItems(
            List<DebitNoteLineType> debitNoteLines) throws PDFReportException {
        if (logger.isDebugEnabled()) {
            logger.debug("+getDebitNoteItems() [" + this.docUUID
        ...
        if (null != debitNoteLines && 0 < debitNoteLines.size()) {
            ...
            try {
                for (DebitNoteLineType dnLine : debitNoteLines) {
                    ...
                    debitNoteItemObj.setQuantityItem(dnLine
                            .getDebitedQuantity().getValue());
                    // debitNoteItemObj.setQuantityItem(dnLine.getDebitedQuantity().getValue().setScale(IPDFCreatorConfig.DECIMAL_ITEM_QUANTITY,
                    // RoundingMode.HALF_UP).toString());
                    debitNoteItemObj.setUnitMeasureItem((null != dnLine
                            .getNote()) ? dnLine.getNote().get(0).getValue() : "");
                    debitNoteItemObj.setDescriptionItem(dnLine.getItem()
                            .getDescription().get(0).getValue().toUpperCase());
                    debitNoteItemObj.setUnitValueItem(dnLine.getPrice()
                            .getPriceAmount().getValue());
                    // debitNoteItemObj.setUnitValueItem(getCurrencyV2(dnLine.getPrice().getPriceAmount().getValue(),
                    // null, IPDFCreatorConfig.PATTERN_FLOAT_DEC_3));

                    BigDecimal unitPrice = getPricingReferenceValue(dnLine
                                    .getPricingReference()
                                    .getAlternativeConditionPrice(),
                            IPDFCreatorConfig.ALTERNATIVE_COND_UNIT_PRICE);
                    ...
                    debitNoteItemObj.setAmountItem(getCurrency(dnLine
                            .getLineExtensionAmount().getValue(), null));

                    itemList.add(debitNoteItemObj);
                }
            } catch (PDFReportException e) {
                logger.error("getDebitNoteItems() [" + this.docUUID
                        + "] ERROR: " + e.getMessage());
                throw e;
            } catch (Exception e) {
                logger.error("getDebitNoteItems() [" + this.docUUID
            ...
            logger.error("getDebitNoteItems() [" + this.docUUID + "] ERROR: "
                    + IVenturaError.ERROR_411.getMessage());
