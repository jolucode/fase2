...
public class UBLDocumentHandler20 {
    ...
    public List<SUNATRetentionDocumentReferenceType> generateDocumentReferenceV2(Set<TransaccionComprobantePago> tscp) throws UBLDocumentException {
        ...
        for (TransaccionComprobantePago transaccionComprobantePago : tscp) {
            ...
            totalInvoiceAmountType.setValue(transaccionComprobantePago.getDOCImporte().setScale(2, BigDecimal.ROUND_HALF_UP));
            ...
            objPaidAmountType.setValue(transaccionComprobantePago.getPagoImporteSR().setScale(2, BigDecimal.ROUND_HALF_UP));
            ...
            objPerceptionAmountType.setValue(transaccionComprobantePago.getCPImporte().setScale(2, BigDecimal.ROUND_HALF_UP));
            ...
            for (TransaccionComprobantePago comprobantePago : tscp) {
                objCashedType.setCurrencyID(CurrencyCodeContentType.valueOf(comprobantePago.getCPMonedaMontoNeto()).value());
                break;
            }
            objCashedType.setValue(transaccionComprobantePago.getCPImporteTotal().setScale(2, BigDecimal.ROUND_HALF_UP));
    ...
    public List<SUNATPerceptionDocumentReferenceType> generateDocumentReference(Set<TransaccionComprobantePago> tscp) throws UBLDocumentException {
        List<SUNATPerceptionDocumentReferenceType> sunatPerceptionDocumentReferenceTypes = new ArrayList<>();
        ...
        for (TransaccionComprobantePago transaccionComprobantePago : tscp) {
            ...
            for (TransaccionComprobantePago comprobantePago : tscp) {
                if (logger.isInfoEnabled()) {
                    logger.info("sac:SUNATPerceptionInformation() - SUNATPerceptionAmount " + comprobantePago.getCPImporte());
                }
                SUNATPerceptionAmountType objPerceptionAmountType = new SUNATPerceptionAmountType();
                objPerceptionAmountType.setCurrencyID(CurrencyCodeContentType.valueOf(comprobantePago.getCPMoneda()).value());
                objPerceptionAmountType.setValue(comprobantePago.getCPImporte().setScale(2));
    ...
    public RetentionType generateRetentionType(Transaccion transaction, String signerName) throws UBLDocumentException {
        ...
        try {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("generatePerceptionType() [" + this.identifier + "] Agregando DOC_Id: " + transaction.getDOCId());
            }
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("generatePerceptionType() [" + this.identifier + "] Agregando IssueDate: " + transaction.getDOCFechaEmision());
            }
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("generatePerceptionType() [" + this.identifier + "] Agregando AgentParty: ");
            }

            /* Agregar <Perception><cac:AgentParty> */
            retentionType.setAgentParty(generatePartyPerception(transaction.getDocIdentidadNro(), transaction.getRazonSocial(), transaction.getNombreComercial(), transaction.getDIRDireccion(), transaction.getDIRDepartamento(), transaction.getDIRProvincia(), transaction.getDIRDistrito(), transaction.getDIRUbigeo(), transaction.getDIRPais(), transaction.getPersonContacto(), transaction.getEMail(), transaction.getDocIdentidadTipo()));

            if (logger.isDebugEnabled()) {
                logger.debug("generatePerceptionType() [" + this.identifier + "] Agregando ReceiverParty: ");
            }

            /* Agregar <Perception><cac:ReceiverParty> */
            retentionType.setReceiverParty(generatePartyPerception(transaction.getSNDocIdentidadNro(), transaction.getSNRazonSocial(), transaction.getSNNombreComercial(), transaction.getSNDIRDireccion(), transaction.getSNDIRDepartamento(), transaction.getSNDIRProvincia(), transaction.getSNDIRDistrito(), transaction.getSNDIRUbigeo(), transaction.getSNDIRPais(), transaction.getPersonContacto(), transaction.getSNEMail(), transaction.getSNDocIdentidadTipo()));

            if (logger.isDebugEnabled()) {
                logger.debug("generatePerceptionType() [" + this.identifier + "] Agregando sunatRetentionSystemCodeType: ");
            }
            ...
            retentionType.setSunatRetentionSystemCode(sunatRetentionSystemCodeType);

            if (logger.isDebugEnabled()) {
                logger.debug("generatePerceptionType() [" + this.identifier + "] Agregando sunatRetentionPercentType: " + transaction.getRETTasa());
            }
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("generatePerceptionType() [" + this.identifier + "] Agregando totalInvoiceAmountType: ");
            }
            TotalInvoiceAmountType totalInvoiceAmountType = new TotalInvoiceAmountType();
            totalInvoiceAmountType.setValue(bg);
            totalInvoiceAmountType.setCurrencyID(CurrencyCodeContentType.valueOf(transaction.getDOCMONCodigo()).value());
            retentionType.setTotalInvoiceAmount(totalInvoiceAmountType);
            if (logger.isDebugEnabled()) {
                logger.debug("generatePerceptionType() [" + this.identifier + "] Agregando totalPaidType: ");
            }
            ...
            totalPaidType.setCurrencyID(CurrencyCodeContentType.valueOf(transaction.getMonedaPagado()).value());
            retentionType.setTotalPaid(totalPaidType);
            retentionType.getSunatRetentionDocumentReference().addAll(generateDocumentReferenceV2(transaction.getTransaccionComprobantePagos()));
    ...
    public PerceptionType generatePerceptionType(Transaccion transaction, String signerName) throws UBLDocumentException {
        ...
        try {
            ...
            /* Agregar <Perception><cac:AgentParty> */
            perceptionType.setAgentParty(generatePartyPerception(transaction.getDocIdentidadNro(), transaction.getRazonSocial(), transaction.getNombreComercial(), transaction.getDIRDireccion(), transaction.getDIRDepartamento(), transaction.getDIRProvincia(), transaction.getDIRDistrito(), transaction.getDIRUbigeo(), transaction.getDIRPais(), transaction.getPersonContacto(), transaction.getEMail(), transaction.getDocIdentidadTipo()));
            if (logger.isDebugEnabled()) {
                logger.debug("generatePerceptionType() - ReceiverParty ");
            }
            /* Agregar <Perception><cac:ReceiverParty> */
            perceptionType.setReceiverParty(generatePartyPerception(transaction.getSNDocIdentidadNro(), transaction.getSNRazonSocial(), transaction.getSNNombreComercial(), transaction.getSNDIRDireccion(), transaction.getSNDIRDepartamento(), transaction.getSNDIRProvincia(), transaction.getSNDIRDistrito(), transaction.getSNDIRUbigeo(), transaction.getSNDIRPais(), transaction.getPersonContacto(), transaction.getSNEMail(), transaction.getSNDocIdentidadTipo()));

            if (logger.isDebugEnabled()) {
                logger.debug("generatePerceptionType() - SunatPerceptionSystemCode" + transaction.getRETRegimen());
            }
            SUNATPerceptionSystemCodeType objSunatPerceptionSystemCodeType = new SUNATPerceptionSystemCodeType();
            objSunatPerceptionSystemCodeType.setValue(transaction.getRETRegimen());
            perceptionType.setSunatPerceptionSystemCode(objSunatPerceptionSystemCodeType);

            if (logger.isDebugEnabled()) {
                logger.debug("generatePerceptionType() - SUNATPerceptionPercent" + transaction.getRETTasa());
            }
            SUNATPerceptionPercentType objPerceptionPercentType = new SUNATPerceptionPercentType();
            objPerceptionPercentType.setValue(transaction.getRETTasa().toString());
            perceptionType.setSunatPerceptionPercent(objPerceptionPercentType);

            if (logger.isDebugEnabled()) {
                logger.debug("generatePerceptionType() - Note  " + transaction.getObservaciones());
            }
            NoteType dueDateNote = new NoteType();
            dueDateNote.setValue(transaction.getObservaciones());
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("generatePerceptionType() -  TotalInvoiceAmount " + transaction.getDOCMontoTotal());
            }
            BigDecimal bg = transaction.getDOCMontoTotal();
            bg = bg.setScale(2, BigDecimal.ROUND_HALF_UP);
            TotalInvoiceAmountType totalInvoiceAmountType = new TotalInvoiceAmountType();
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("generatePerceptionType() -  SUNATNetTotalCashed " + transaction.getImportePagado());
            }
            SUNATTotalCashedType sunatTotalCashedType = new SUNATTotalCashedType();
            sunatTotalCashedType.setValue(transaction.getImportePagado().setScale(2, BigDecimal.ROUND_HALF_UP));
            sunatTotalCashedType.setCurrencyID(CurrencyCodeContentType.valueOf(transaction.getMonedaPagado()).value());
            perceptionType.setSunatTotalCashed(sunatTotalCashedType);

            perceptionType.getSunatPerceptionDocumentReference().addAll(generateDocumentReference(transaction.getTransaccionComprobantePagos()));
    ...
    public InvoiceType generateInvoiceType(Transaccion transaction, String signerName) throws UBLDocumentException {
        ...
        try {
            ...
            UBLExtensionsType ublExtensions = new UBLExtensionsType();
            {
                 ...
                 */
                ublExtensions.getUBLExtension().add(getUBLExtensionTotalAndProperty(transaction.getTransaccionTotalesList(), transaction.getTransaccionPropiedadesList(), transaction.getSUNATTransact()));
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("generateInvoiceType() [" + this.identifier + "] Agregando DOC_Id: " + transaction.getDOCId());
            }
            IDType idDocIdentifier = new IDType();
            idDocIdentifier.setValue(transaction.getDOCId());
            ...
            invoiceType.setInvoiceTypeCode(getInvoiceTypeCode(transaction.getDOCCodigo()));
            ...
            if (null != transaction.getDOCFechaVencimiento()) {
                 ...
                 */
                NoteType dueDateNote = new NoteType();
                dueDateNote.setValue(getDueDateValue(transaction.getDOCFechaVencimiento()));
                invoiceType.getNote().add(dueDateNote);
            }

            /* Agregar <Invoice><cbc:DocumentCurrencyCode> */
            invoiceType.setDocumentCurrencyCode(getDocumentCurrencyCode(transaction.getDOCMONNombre(), transaction.getDOCMONCodigo()));
            ...
            SupplierPartyType accountingSupplierParty = generateAccountingSupplierParty(transaction.getDocIdentidadNro(), transaction.getDocIdentidadTipo(), transaction.getRazonSocial(), transaction.getNombreComercial(), transaction.getDIRDireccion(), transaction.getDIRDepartamento(), transaction.getDIRProvincia(), transaction.getDIRDistrito(), transaction.getDIRUbigeo(), transaction.getDIRPais(), transaction.getPersonContacto(), transaction.getEMail());
            ...
            CustomerPartyType accountingCustomerParty = generateAccountingCustomerParty(transaction.getSNDocIdentidadNro(), transaction.getSNDocIdentidadTipo(), transaction.getSNRazonSocial(), transaction.getSNNombreComercial(), transaction.getSNDIRNomCalle(), transaction.getSNDIRDepartamento(), transaction.getSNDIRProvincia(), transaction.getSNDIRDistrito(), transaction.getSNDIRUbigeo(), transaction.getSNDIRPais(), transaction.getSNSegundoNombre(), transaction.getSNEMail());
             ...
             */
            //SETEAR OTROS CARGOS
            invoiceType.setLegalMonetaryTotal(getMonetaryTotal(transaction.getDOCImporte(), transaction.getDOCMontoTotal(), transaction.getDOCOtrosCargos(), transaction.getANTICIPOMonto(), transaction.getDOCMONCodigo()));

            /* Agregar items <Invoice><cac:InvoiceLine> */
            invoiceType.getInvoiceLine().addAll(getAllInvoiceLines(transaction.getTransaccionLineas(), transaction.getDOCMONCodigo()));
    ...
    public DespatchAdviceType generateDespatchAdviceType(Transaccion transaction, String signerName) throws UBLDocumentException {
        ...
        try {
            ...
            despatchAdviceType.setUBLVersionID(getUBLVersionID12_0());
            //despatchAdviceType.setUBLVersionID(getUBLVersionID1_0());
            ...
            /* Agregar <Invoice><cbc:ID> */
            if (logger.isDebugEnabled()) {
                logger.debug("generateDespatchAdviceType() [" + this.identifier + "] Agregando DOC_Id: " + transaction.getDOCId());
            }
            IDType idDocIdentifier = new IDType();
            idDocIdentifier.setValue(transaction.getDOCId());
            ...
            noteType.setValue(transaction.getObservaciones());
            despatchAdviceType.getNote().add(noteType);

            despatchAdviceType.setDespatchSupplierParty(getDespatchSupplierParty(transaction.getDocIdentidadTipo(), transaction.getDocIdentidadNro(), transaction.getRazonSocial()));
            despatchAdviceType.setDeliveryCustomerParty(getDespatchCustomerParty(transaction.getSNDocIdentidadTipo(), transaction.getSNDocIdentidadNro(), transaction.getSNRazonSocial()));

            despatchAdviceType.setShipment(getShipment(transaction));
            ...
            for (TransaccionLineas transaccionLinea : transaccionLineas) {
                ...
                iDType.setValue(String.valueOf(transaccionLinea.getTransaccionLineasPK().getNroOrden()));
                ...
                lineIDType.setValue(String.valueOf(transaccionLinea.getTransaccionLineasPK().getNroOrden()));
                orderLineReferenceType.setLineID(lineIDType);

                DeliveredQuantityType deliveredQuantityType = new DeliveredQuantityType();
                deliveredQuantityType.setUnitCode(transaccionLinea.getUnidad());
                deliveredQuantityType.setValue(transaccionLinea.getCantidad().setScale(0, RoundingMode.HALF_UP));

                ItemType itemType = new ItemType();
                NameType nameType = new NameType();
                nameType.setValue(transaccionLinea.getDescripcion());
                itemType.setName(nameType);

                ItemIdentificationType itemIdentificationType = new ItemIdentificationType();
                IDType dType = new IDType();
                dType.setValue(transaccionLinea.getCodArticulo());
                itemIdentificationType.setID(dType);
                ...
                despatchLineType.getOrderLineReference().add(orderLineReferenceType);
                despatchAdviceType.getDespatchLine().add(despatchLineType);
            }
    ...
    public InvoiceType generateBoletaType(Transaccion transaction, String signerName) throws UBLDocumentException {
        ...
        try {
            ...
            /* Agregar <Invoice><ext:UBLExtensions> */
            if (logger.isDebugEnabled()) {
                logger.debug("generateBoletaType() [" + this.identifier + "] Agregando TOTALES Y PROPIEDADES.");
            }
            UBLExtensionsType ublExtensions = new UBLExtensionsType();
            {
                /*
            ...
            /* Agregar <Invoice><cbc:ID> */
            if (logger.isDebugEnabled()) {
                logger.debug("generateBoletaType() [" + this.identifier + "] Agregando DOC_Id: " + transaction.getDOCId());
            }
            IDType idDocIdentifier = new IDType();
            idDocIdentifier.setValue(transaction.getDOCId());
            ...
            /* Agregar <Invoice><cbc:InvoiceTypeCode> */
            boletaType.setInvoiceTypeCode(getInvoiceTypeCode(transaction.getDOCCodigo()));
            ...
            /* Agregar <Invoice><cbc:DocumentCurrencyCode> */
            boletaType.setDocumentCurrencyCode(getDocumentCurrencyCode(transaction.getDOCMONNombre(), transaction.getDOCMONCodigo()));
            ...
            if (optional.isPresent()) {
                TransaccionContractdocref docRefer = optional.get();
                boletaType.getContractDocumentReference().add(getContractDocumentReference(docRefer.getValor(), "OC"));
            }
            ...
            if (null != transaction.getTransaccionDocrefersList() && 0 < transaction.getTransaccionDocrefersList().size()) {
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug("generateInvoiceType() [" + this.identifier + "] Se agregaron [" + boletaType.getDespatchDocumentReference().size() + "] guias de remision.");
                }
            }

            /* Agregar impuestos <Invoice><cac:TaxTotal> */
            boletaType.getTaxTotal().addAll(getAllTaxTotal(transaction.getTransaccionImpuestosList()));
             ...
             * la Factura - Monto total de anticipos
             */
            boletaType.setLegalMonetaryTotal(getMonetaryTotal(transaction.getDOCImporte(), transaction.getDOCMontoTotal(), transaction.getDOCOtrosCargos(), transaction.getANTICIPOMonto(), transaction.getDOCMONCodigo()));

            /* Agregar items <Invoice><cac:InvoiceLine> */
            boletaType.getInvoiceLine().addAll(getAllBoletaLines(transaction.getTransaccionLineas(), transaction.getDOCMONCodigo()));
        } catch (UBLDocumentException e) {
    ...
    public CreditNoteType generateCreditNoteType(Transaccion transaction, String signerName) throws UBLDocumentException {
        ...
        try {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("generateCreditNoteType() [" + this.identifier + "] Agregando TOTALES Y PROPIEDADES.");
            }
            ...
            UBLExtensionsType ublExtensions = new UBLExtensionsType();
            {
                 ...
                 * ><sac:AdditionalInformation>
                 */
                ublExtensions.getUBLExtension().add(getUBLExtensionTotalAndPropertyForNCND(transaction.getTransaccionTotalesList(), transaction.getTransaccionPropiedadesList(), transaction.getSUNATTransact()));
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("generateCreditNoteType() [" + this.identifier + "] Agregando DOC_Id: " + transaction.getDOCId());
            }
            IDType idDocIdentifier = new IDType();
            idDocIdentifier.setValue(transaction.getDOCId());
            ...
            if (null != transaction.getDOCFechaVencimiento()) {
                if (logger.isInfoEnabled()) {
                    logger.info("generateCreditNoteType() [" + this.identifier + "] La transaccion contiene FECHA DE VENCIMIENTO.");
                }
                /*
                 * Agregar una nota para colocar la fecha de vencimiento.
                 */
            ...
            creditNoteType.setDocumentCurrencyCode(getDocumentCurrencyCode(transaction.getDOCMONNombre(), transaction.getDOCMONCodigo()));
            ...
            if (null != transaction.getTransaccionDocrefersList() && 0 < transaction.getTransaccionDocrefersList().size()) {
                ...
                creditNoteType.getDespatchDocumentReference().addAll(getDespatchDocumentReferences(transaction.getTransaccionDocrefersList()));
                if (logger.isDebugEnabled()) {
                    logger.debug("generateInvoiceType() [" + this.identifier + "] Se agregaron [" + creditNoteType.getDespatchDocumentReference().size() + "] guias de remision.");
                }
            }
            ...
            if (StringUtils.isNotBlank(transaction.getDOCCondPago())) {
                ...
                creditNoteType.getContractDocumentReference().add(getContractDocumentReference(transaction.getDOCCondPago(), IUBLConfig.CONTRACT_DOC_REF_PAYMENT_COND_CODE));
            }
            ...
            if (optional.isPresent()) {
                TransaccionContractdocref docRefer = optional.get();
                creditNoteType.getContractDocumentReference().add(getContractDocumentReference(docRefer.getValor(), "OC"));
            }


            /* Agregar <CreditNote><cac:DiscrepancyResponse> */
            creditNoteType.getDiscrepancyResponse().add(getDiscrepancyResponse(transaction.getREFDOCId(), transaction.getREFDOCMotivCode(), transaction.getREFDOCMotivDesc()));
            ...
            if (transaction.getDOCId().startsWith(IUBLConfig.INVOICE_SERIE_PREFIX)) {
                if (logger.isInfoEnabled()) {
                    logger.info("generateCreditNoteType() [" + this.identifier + "] Generando RECEPTOR de FACTURA.");
                }

                CustomerPartyType accountingCustomerParty = generateAccountingCustomerParty(transaction.getSNDocIdentidadNro(), transaction.getSNDocIdentidadTipo(), transaction.getSNRazonSocial(), transaction.getSNNombreComercial(), transaction.getSNDIRNomCalle(), transaction.getSNDIRDepartamento(), transaction.getSNDIRProvincia(), transaction.getSNDIRDistrito(), transaction.getSNDIRUbigeo(), transaction.getSNDIRPais(), transaction.getSNSegundoNombre(), transaction.getSNEMail());
                creditNoteType.setAccountingCustomerParty(accountingCustomerParty);

            } else if (transaction.getDOCId().startsWith(IUBLConfig.BOLETA_SERIE_PREFIX)) {
                if (logger.isInfoEnabled()) {
                    logger.info("generateCreditNoteType() [" + this.identifier + "] Generando RECEPTOR de BOLETA.");
                }
                CustomerPartyType accountingCustomerParty = generateAccountingCustomerPartyV2(transaction.getSNDocIdentidadNro(), transaction.getSNDocIdentidadTipo(), transaction.getSNRazonSocial(), transaction.getSNDIRNomCalle(), transaction.getSNDIRDepartamento(), transaction.getSNDIRProvincia(), transaction.getSNDIRDistrito(), transaction.getSNSegundoNombre(), transaction.getSNEMail());
                creditNoteType.setAccountingCustomerParty(accountingCustomerParty);
            } else {
                logger.error("generateCreditNoteType() [" + this.identifier + "] ERROR: " + IVenturaError.ERROR_339.getMessage());
                throw new UBLDocumentException(IVenturaError.ERROR_339);
            }

            /* Agregar impuestos <CreditNote><cac:TaxTotal> */
            creditNoteType.getTaxTotal().addAll(getAllTaxTotal(transaction.getTransaccionImpuestosList()));
             ...
             */
            creditNoteType.setLegalMonetaryTotal(getMonetaryTotal(transaction.getDOCImporte(), transaction.getDOCMontoTotal(), transaction.getDOCOtrosCargos(), null, transaction.getDOCMONCodigo()));
            ...
            creditNoteType.getCreditNoteLine().addAll(getAllCreditNoteLines(transaction.getTransaccionLineas(), transaction.getDOCMONCodigo()));
        } catch (UBLDocumentException e) {
    ...
    public DebitNoteType generateDebitNoteType(Transaccion transaction, String signerName) throws UBLDocumentException {
        ...
        try {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("generateDebitNoteType() [" + this.identifier + "] Agregando TOTALES Y PROPIEDADES.");
            }
            BigDecimal subTotalValue = getSubtotalValueFromTransaction(transaction.getTransaccionTotalesList());
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("generateDebitNoteType() [" + this.identifier + "] Agregando DOC_Id: " + transaction.getDOCId());
            }
            IDType idDocIdentifier = new IDType();
            idDocIdentifier.setValue(transaction.getDOCId());
            ...
            if (null != transaction.getDOCFechaVencimiento()) {
                if (logger.isInfoEnabled()) {
                    logger.info("generateDebitNoteType() [" + this.identifier + "] La transaccion contiene FECHA DE VENCIMIENTO.");
                }
                /*
                 * Agregar una nota para colocar la fecha de vencimiento.
                 */
            ...
            debitNoteType.setDocumentCurrencyCode(getDocumentCurrencyCode(transaction.getDOCMONNombre(), transaction.getDOCMONCodigo()));

            /* Agregar <DebitNote><cac:DiscrepancyResponse> */
            debitNoteType.getDiscrepancyResponse().add(getDiscrepancyResponse(transaction.getREFDOCId(), transaction.getREFDOCMotivCode(), transaction.getREFDOCMotivDesc()));
            ...
            if (transaction.getDOCId().startsWith(IUBLConfig.INVOICE_SERIE_PREFIX)) {
                if (logger.isInfoEnabled()) {
                    logger.info("generateDebitNoteType() [" + this.identifier + "] Generando RECEPTOR de FACTURA.");
                }
                CustomerPartyType accountingCustomerParty = generateAccountingCustomerParty(transaction.getSNDocIdentidadNro(), transaction.getSNDocIdentidadTipo(), transaction.getSNRazonSocial(), transaction.getSNNombreComercial(), transaction.getSNDIRNomCalle(), transaction.getSNDIRDepartamento(), transaction.getSNDIRProvincia(), transaction.getSNDIRDistrito(), transaction.getSNDIRUbigeo(), transaction.getSNDIRPais(), transaction.getSNSegundoNombre(), transaction.getSNEMail());
                debitNoteType.setAccountingCustomerParty(accountingCustomerParty);
            } else if (transaction.getDOCId().startsWith(IUBLConfig.BOLETA_SERIE_PREFIX)) {
                if (logger.isInfoEnabled()) {
                    logger.info("generateDebitNoteType() [" + this.identifier + "] Generando RECEPTOR de BOLETA.");
                }
                ...
                logger.error("generateDebitNoteType() [" + this.identifier + "] ERROR: " + IVenturaError.ERROR_339.getMessage());
                throw new UBLDocumentException(IVenturaError.ERROR_339);
            }

            /* Agregar impuestos <DebitNote><cac:TaxTotal> */
            debitNoteType.getTaxTotal().addAll(getAllTaxTotal(transaction.getTransaccionImpuestosList()));
             ...
             * la Factura - NO EXISTE ANTICIPOS
             */
            debitNoteType.setRequestedMonetaryTotal(getMonetaryTotal(transaction.getDOCImporte(), transaction.getDOCMontoTotal(), transaction.getDOCOtrosCargos(), null, transaction.getDOCMONCodigo()));
            ...
            debitNoteType.getDebitNoteLine().addAll(getAllDebitNoteLines(transaction.getTransaccionLineas(), transaction.getDOCMONCodigo()));
        } catch (UBLDocumentException e) {
    ...
    public VoidedDocumentsType generateReversionDocumentType(Transaccion transaction, String signerName) throws UBLDocumentException {
        ...
        try {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("generateVoidedDocumentType() [" + this.identifier + "] Agregando TAG para colocar la FIRMA.");
            }
            UBLExtensionsType ublExtensions = new UBLExtensionsType();
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("generateVoidedDocumentType() [" + this.identifier + "] Agregando Identificador: " + transaction.getANTICIPOId());
            }
            IDType idDocIdentifier = new IDType();
            ...
            voidedDocumentType.setIssueDate(getIssueDate(transaction.getDOCFechaVencimiento()));
            ...
            SupplierPartyType accountingSupplierParty = generateAccountingSupplierPartyV2(transaction.getDocIdentidadNro(), transaction.getDocIdentidadTipo(), transaction.getRazonSocial(), transaction.getNombreComercial());
...
//    }
    ...
    private UBLExtensionType getUBLExtensionTotalAndProperty(Set<TransaccionTotales> transactionTotalList, Set<TransaccionPropiedades> transactionPropertyList, String sunatTransactionID) throws UBLDocumentException {
        if (logger.isDebugEnabled()) {
        ...
        try {
            ...
            if (null == transactionTotalList) {
                ...
                for (TransaccionTotales transactionTotal : transactionTotalList) {
                    ...
                    payableAmount.setCurrencyID(CurrencyCodeContentType.valueOf(transactionTotal.getTransaccion().getDOCMONCodigo()).value());
    ...
    private UBLExtensionType getUBLExtensionTotalAndPropertyForNCND(Set<TransaccionTotales> transactionTotalList, Set<TransaccionPropiedades> transactionPropertyList, String sunatTransactionID) throws UBLDocumentException {
        if (logger.isDebugEnabled()) {
            logger.debug("+getUBLExtensionTotalAndProperty() [" + this.identifier + "] transactionTotalList: " + transactionTotalList + " transactionPropertyList: " + transactionPropertyList + " sunatTransactionID: " + sunatTransactionID);
        }
        UBLExtensionType ublExtension = null;
    ...
    private List<PaymentType> generatePrepaidPaymentV2(Set<TransaccionAnticipo> lstAnticipo) throws UBLDocumentException {
        ...
        if (lstAnticipo != null) {
            ...
            for (TransaccionAnticipo transaccionAnticipo : lstAnticipo) {
                ...
                try {
                    ...
                    if (logger.isDebugEnabled()) {
                        logger.debug("+generatePrepaidPayment() [" + transaccionAnticipo.getAnticipoMonto() + transaccionAnticipo.getDOCMoneda() + "]");
                    }
                    if (logger.isDebugEnabled()) {
                    ...
                    if (logger.isDebugEnabled()) {
                        logger.debug("+generatePrepaidPayment() [" + transaccionAnticipo.getAntiDOCSerieCorrelativo() + transaccionAnticipo.getAntiDOCTipo() + "]");
                    }
                    ...
                    if (logger.isDebugEnabled()) {
                        logger.debug("+generatePrepaidPayment() [" + transaccionAnticipo.getDOCNumero() + transaccionAnticipo.getDOCTipo() + "]");
                    }
                    /*
                     * Agregar los tag's
                     */
    ...
    private List<TaxTotalType> getAllTaxTotal(Set<TransaccionImpuestos> taxTotalTransactions) throws UBLDocumentException {
        if (logger.isDebugEnabled()) {
    ...
    private List<TaxTotalType> getAllTaxTotalLines(Set<TransaccionLineaImpuestos> taxTotalTransactionLines) throws UBLDocumentException {
        if (logger.isDebugEnabled()) {
    ...
    private List<InvoiceLineType> getAllInvoiceLines(Set<TransaccionLineas> transactionLines, String currencyCode) throws UBLDocumentException {
        if (logger.isDebugEnabled()) {
    ...
    private List<InvoiceLineType> getAllBoletaLines(Set<TransaccionLineas> transactionLines, String currencyCode) throws UBLDocumentException {
        ...
        if (null != transactionLines && !transactionLines.isEmpty()) {
            try {
                boletaLineList = new ArrayList<>(transactionLines.size());
                int i = 0;
                for (TransaccionLineas transactionLine : transactionLines) {
                    IUBLConfig.lstImporteIGV.add(i++, transactionLine.getTotalLineaConIGV());
                }
                ...
                for (BigDecimal bigDecimal : IUBLConfig.lstImporteIGV) {
                    if (logger.isDebugEnabled()) {
                        logger.debug("getAllBoletaLinesImporteIGV." + bigDecimal);
                    }
                }
    ...
    private List<CreditNoteLineType> getAllCreditNoteLines(Set<TransaccionLineas> transactionLines, String currencyCode) throws UBLDocumentException {
        if (logger.isDebugEnabled()) {
    ...
    private List<DebitNoteLineType> getAllDebitNoteLines(Set<TransaccionLineas> transactionLines, String currencyCode) throws UBLDocumentException {
        if (logger.isDebugEnabled()) {
    ...
    private PriceType getPriceForLine(Set<TransaccionLineasBillref> transBillReferenceList, String currencyCode) throws UBLDocumentException {
        if (logger.isDebugEnabled()) {
        ...
        try {
            ...
            if (null != transBillReferenceList && 0 < transBillReferenceList.size()) {
                ...
                if (StringUtils.isNotBlank(unitValue)) {
                    ...
                    priceAmount.setValue(BigDecimal.valueOf(Double.parseDouble(unitValue)).setScale(IUBLConfig.DECIMAL_LINE_UNIT_VALUE, RoundingMode.HALF_UP));
    ...
    private BigDecimal getSubtotalValueFromTransaction(Set<TransaccionTotales> transactionTotalList) throws UBLDocumentException {
    ...
    private List<DocumentReferenceType> getDespatchDocumentReferences(Set<TransaccionDocrefers> transaccionDocrefersList) {
        if (logger.isInfoEnabled()) {
    ...
    private ShipmentType getShipment(Transaccion tr) {
        ...
        iDTypeSN.setValue(tr.getSNDIRUbigeo());
        addressSN.setID(iDTypeSN);
        StreetNameType streetNameTypeSN = new StreetNameType();
        streetNameTypeSN.setValue(tr.getSNDIRDireccion());
