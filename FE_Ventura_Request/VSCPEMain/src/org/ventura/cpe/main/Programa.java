...
import org.ventura.cpe.request.VSCPERequest;
...
//            if (!InstanciaUnica(PuertoVSCPE.PuertoAbierto_Request)) {
//                JOptionPane.showMessageDialog(null, "Existe otra instancia de la aplicación ejectandose.");
//                System.exit(0);
//                return;
//            } else {
//                LoggerTrans.getCDMainLogger().log(Level.CONFIG, "Puerto encontrado {0}", PuertoVSCPE.PuertoAbierto_Request);
//            }
            LoggerTrans.getCDMainLogger().log(Level.CONFIG, "Puerto encontrado {0}", PuertoVSCPE.PuertoAbierto_Request);

            String mensaje = DocumentoINF.Conector.Conectar();
//            String mensaje = "Ventura Soluciones S.A.C.";
            SAPSociedad = mensaje;
            if (mensaje.startsWith("ERR:")) {
                ...
                if (DocumentoBL.mapSociedades != null && DocumentoBL.mapSociedades.size() > 0) {
                    mensaje = "Conectado a " + (DocumentoBL.mapSociedades.size()) + " Sociedad(s)";
                }
                LoggerTrans.getCDMainLogger().log(Level.INFO, "Conectado a {0} ", mensaje);
                VSCPELoader.procesar();
                VSCPERequest.procesar();
    ...
    public static void stop() {
        Thread T = new Thread(() -> {
            ...
            try {
                ...
                LoggerTrans.cerrarLogger("Request_Request");
                LoggerTrans.cerrarLogger("Request_Loader");
                LoggerTrans.cerrarLogger("Request_Main");
            }
        });
        T.start();
    }
